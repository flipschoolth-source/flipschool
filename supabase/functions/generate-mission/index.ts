...
serve(async (req) => {
  ...
  try {
    ...
    const html = `
      ...
      </body></html>`;import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
...
ไม่ใช้ markdown
    ...
    if (!text) throw new Error("Gemini ไม่ได้ส่งเนื้อหากลับมา");
...
<meta charset="UTF-8">
<style>
body{font-family:Sarabun,sans-serif;padding:20px;line-height:1.7}
h1{color:#7c3aed}
h2{margin-top:20px}
</style>
</head>
<body>
${text
  .replace(/ชื่อบทเรียน:/g, "<h1>📘 ชื่อบทเรียน</h1>")
  .replace(/คำอธิบาย:/g, "<h2>📝 คำอธิบาย</h2>")
  .replace(/เนื้อหา:/g, "<h2>📚 เนื้อหา</h2>")
  .replace(/กิจกรรม:/g, "<h2>🎯 กิจกรรม</h2>")
  .replace(/คำถามท้ายบท:/g, "<h2>❓ คำถามท้ายบท</h2>")
  .replace(/\n/g, "<br>")
}
</body>
</html>
`;
    ...
    return new Response(
      JSON.stringify({
        html: `<div style="color:red;font-family:Sarabun">
